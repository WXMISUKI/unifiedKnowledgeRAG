...
def main() -> None:
    ...
    print(f"Phase 12d pgvector live probe readiness JSON ready: {report.json_path}")
    print(
        "Phase 12d pgvector live probe readiness Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 12d pgvector live probe readiness report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/operations/pgvector-live-probe-readiness"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
