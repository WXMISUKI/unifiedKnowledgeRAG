...
def main() -> None:
    ...
    print(f"Phase 12b candidate backend evaluation readiness JSON ready: {report.json_path}")
    print(
        "Phase 12b candidate backend evaluation readiness Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 12b candidate backend evaluation readiness report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/operations/candidate-backend-evaluation-readiness"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
