...
def main() -> None:
    ...
    print(f"Phase 3 readiness JSON ready: {report.json_path}")
    print(f"Phase 3 readiness Markdown ready: {report.markdown_path}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 3 retrieval promotion readiness report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/benchmark/chinese-seed/retrieval-promotion-readiness"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
