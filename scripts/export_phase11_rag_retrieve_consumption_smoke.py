...
def main() -> None:
    args = _parse_args()
    report = export_phase11_rag_retrieve_consumption_smoke_report(
        output_dir=args.output_dir,
        base_dir=args.artifact_base_dir,
    )
    print(f"Phase 11 retrieve-consumption smoke JSON ready: {report.json_path}")
    print(f"Phase 11 retrieve-consumption smoke Markdown ready: {report.markdown_path}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export Phase 11 RAG retrieve consumption smoke."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
