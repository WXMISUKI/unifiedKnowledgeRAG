...
def main() -> None:
    ...
    print(
        "Phase 3 hybrid runtime promotion decision smoke JSON ready: "
        f"{report.json_path}"
    )
    print(
        "Phase 3 hybrid runtime promotion decision smoke Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 3 hybrid runtime promotion decision smoke report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/smoke/hybrid-runtime-promotion"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
