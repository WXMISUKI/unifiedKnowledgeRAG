...
def main() -> None:
    ...
    print(
        "Phase 7 cross-phase handoff consistency smoke JSON ready: "
        f"{report.json_path}"
    )
    print(
        "Phase 7 cross-phase handoff consistency smoke Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 7 cross-phase handoff consistency smoke report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/smoke/cross-phase-handoff"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
