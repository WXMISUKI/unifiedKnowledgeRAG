...
def main() -> None:
    ...
    print(
        "Phase 12f pgvector local live probe execution readiness JSON ready: "
        f"{report.json_path}"
    )
    print(
        "Phase 12f pgvector local live probe execution readiness Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 12f pgvector live probe execution readiness report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/operations/pgvector-local-live-probe-execution"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
