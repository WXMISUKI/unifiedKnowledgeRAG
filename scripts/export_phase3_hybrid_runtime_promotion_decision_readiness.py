...
def main() -> None:
    ...
    print(f"Phase 3 hybrid runtime promotion readiness JSON ready: {report.json_path}")
    print(
        "Phase 3 hybrid runtime promotion readiness Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export the local Phase 3 hybrid runtime promotion decision "
            "readiness report."
        )
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/benchmark/chinese-seed/hybrid-runtime-promotion"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
