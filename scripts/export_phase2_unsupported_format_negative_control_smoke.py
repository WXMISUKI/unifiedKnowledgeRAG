...
def main() -> None:
    ...
    print(
        "Phase 2 unsupported-format negative-control smoke JSON ready: "
        f"{report.json_path}"
    )
    print(
        "Phase 2 unsupported-format negative-control smoke Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 2 unsupported-format negative-control smoke report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/smoke/source-format-demand"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
