...
def main() -> None:
    ...
    print(f"Phase 3 hybrid calibration JSON ready: {report.json_path}")
    print(f"Phase 3 hybrid calibration Markdown ready: {report.markdown_path}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 3 hybrid fusion/threshold calibration report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
