...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate onboarding template path with source_template_example.",
    )
    ...
    example_json = output_dir / "source-template-local-business-rag-golden-cases.json"
    example_md = output_dir / "source-template-local-business-rag-golden-cases.md"
    if example_json.exists():
        example_json.unlink()
    if example_md.exists():
        example_md.unlink()
    report.json_path.rename(example_json)
    report.markdown_path.rename(example_md)

    print(f"Onboarding JSON ready: {onboarding_report.json_path}")
    print(f"Baseline fixture: {case_file}")
    print(f"Validation JSON ready: {example_json}")
    print(f"Validation Markdown ready: {example_md}")
