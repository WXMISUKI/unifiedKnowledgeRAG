...
def main() -> None:
    ...
    print(f"Phase 10 local consumer readiness JSON ready: {report.json_path}")
    print(f"Phase 10 local consumer readiness Markdown ready: {report.markdown_path}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export Phase 10 MyPrivateAgent local consumer readiness."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/integration/myprivateagent-local-consumer-verification"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
