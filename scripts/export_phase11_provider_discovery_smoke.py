...
def main() -> None:
    ...
    print(f"Phase 11 provider discovery smoke JSON ready: {report.json_path}")
    print(f"Phase 11 provider discovery smoke Markdown ready: {report.markdown_path}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export Phase 11 provider discovery smoke."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
