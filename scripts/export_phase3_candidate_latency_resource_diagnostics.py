...
def main() -> None:
    ...
    print(f"Phase 3 latency/resource diagnostics JSON ready: {report.json_path}")
    print(f"Phase 3 latency/resource diagnostics Markdown ready: {report.markdown_path}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 3 candidate latency/resource diagnostics report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
