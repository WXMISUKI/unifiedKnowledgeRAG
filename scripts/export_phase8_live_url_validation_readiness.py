...
def main() -> None:
    ...
    print(f"Phase 8 live URL validation readiness JSON ready: {report.json_path}")
    print(
        "Phase 8 live URL validation readiness Markdown ready: "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export the local Phase 8 live URL validation readiness report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/operations/live-url-validation"),
    )
    parser.add_argument(
        "--artifact-base-dir",
        type=Path,
