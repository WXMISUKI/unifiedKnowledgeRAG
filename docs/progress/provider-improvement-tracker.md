...
8. Next recommended provider slice: add a very small evidence-only bridge from the source onboarding catalog into the existing source evaluation pack catalog, or convert the remaining template-only onboarding example into a real baseline fixture; keep query rewrite, rerank, hybrid retrieval, RAPTOR, Self/Corrective RAG, and GraphRAG outside scope until repeated accepted failure classes justify them.
