...
8. Next recommended provider slice: add a light registration/discovery bridge from onboarding outputs into the source evaluation pack catalog, or validate one more clearly distinct real source shape only if that improves provider-general onboarding confidence; keep query rewrite, rerank, hybrid retrieval, RAPTOR, Self/Corrective RAG, and GraphRAG outside scope until repeated accepted failure classes justify them.
