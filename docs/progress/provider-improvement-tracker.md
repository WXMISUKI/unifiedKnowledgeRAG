...
- Caller trial execution task pack documented (`document-caller-trial-execution-task-pack`), turning stage 2 into a concrete execution aid with pre-trial checks, required outputs, and provider feedback handoff expectations.
- Phase 25 follow-up decision matrix documented (`document-phase25-followup-decision-matrix`), turning stage 3 into a concrete post-trial decision entrypoint for hold-state, review classification, or focused provider reopen.
...
- Real-source onboarding validation implemented (`validate-onboarding-template-with-one-real-source`), using `split_refund_policy_docs` as a new real source proof; the resulting baseline report returns `decision=go` with `case_count=3`, `hit_rate=1.0`, `citation_match_rate=1.0`, and `empty_handling_rate=1.0`, confirming that the template-driven onboarding path works beyond the original three business-rag sources.
...
- `split_refund_policy_docs` onboarding validation artifacts are now available under `docs/local-run/business-rag-golden-cases/onboarding/split_refund_policy_docs/`; use them as the first real-source proof of the template path before changing registration or catalog discovery behavior.
...
- Third distinct real-source onboarding validation implemented (`validate-onboarding-template-with-third-distinct-real-source`), proving the standard onboarding template path on a lightweight rule-oriented markdown source `invoice_policy_faq` with `decision=go`, `case_count=3`, `hit_rate=1.0`, `citation_match_rate=1.0`, and `empty_handling_rate=1.0` while preserving runtime defaults and keeping the result outside aggregate expansion.
- Source onboarding catalog discovery bridge implemented (`add-onboarding-catalog-discovery-bridge`), adding a unified evidence-only view over onboarding sources with `source_count=3`, `ready_source_count=2`, and `template_only_source_count=1` while preserving runtime defaults, avoiding source registration, and avoiding aggregate-baseline expansion.
- Pack-catalog onboarding summary bridge implemented (`bridge-onboarding-catalog-into-pack-catalog`), allowing the source evaluation pack catalog to expose onboarding maturity summary (`onboarding_source_count=3`, `onboarding_ready_source_count=2`, `onboarding_template_only_source_count=1`) without changing pack decision semantics, runtime defaults, source registration, or aggregate-baseline expansion.
- Template onboarding example promoted to a real minimal baseline (`promote-template-onboarding-example-to-real-baseline`), turning `source_template_example` from `template_only` into a `go` onboarding-ready example with `case_count=3`, `hit_rate=1.0`, `citation_match_rate=1.0`, and `empty_handling_rate=1.0` while preserving runtime defaults and keeping the source outside aggregate-baseline expansion.
- Provider next-step trigger contract documented (`document-provider-next-step-trigger-contract`), freezing the current posture as `paused_for_provider_feature_expansion_until_stronger_trigger_appears` and requiring future provider changes to declare `real_caller_feedback_trigger`, `provider_owned_gap_trigger`, `repeated_cross_source_failure_class_trigger`, or `runtime_strategy_evaluation_trigger`.
...
- Use the caller trial feedback runbook as the first execution entrypoint after provider closure instead of opening another provider-side feature slice.
- Use the caller trial execution task pack when switching from stage-1 access review into stage-2 real caller trial execution.
- Use the Phase 25 follow-up decision matrix when switching from stage-2 outcome export into stage-3 provider-side follow-up classification.
