...
8. Next recommended provider slice: wait for a repeated cross-source failed-question class before opening a narrowly scoped hardening slice, or pause provider-side expansion until a real caller exposes a concrete provider-owned gap; keep query rewrite, rerank, hybrid retrieval, RAPTOR, Self/Corrective RAG, and GraphRAG outside scope until repeated accepted failure classes justify them.
