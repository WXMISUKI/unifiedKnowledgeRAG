...
The current aggregate report is `review` with `source_count=2`, `case_count=9`, `hit_rate=1.0`, `citation_match_rate=1.0`, and `empty_handling_rate=0.6667`. The newly added second real source is `refund_policy_docs`. Its exact-term positive controls pass, but the source currently exposes a real negative-control leakage case and `page_coverage_missing` chunk-quality review signal, so the aggregate baseline now carries concrete second-source failure evidence instead of a single-source green path.

Because the aggregate evidence now has a real reviewed source, the next provider action is to classify and address the observed failure modes before considering any advanced RAG strategy change. The immediate trigger is still evidence-driven: review the second-source negative-control leakage and provenance/chunk-quality signal first, then decide whether the next slice belongs to chunking/provenance review, query mismatch review, or another concrete gate. Advanced RAG strategy changes remain unpromoted until that failure-class decision is explicit.
