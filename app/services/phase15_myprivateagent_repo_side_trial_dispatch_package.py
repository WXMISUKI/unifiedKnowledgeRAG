...
PHASE15_MYPRIVATEAGENT_REPO_SIDE_TRIAL_DISPATCH_PACKAGE_ID = (
    "phase15-myprivateagent-repo-side-trial-dispatch-package-v1"
)
OUTPUT_JSON_FILENAME = "phase15-myprivateagent-repo-side-trial-dispatch-package.json"
OUTPUT_MARKDOWN_FILENAME = "phase15-myprivateagent-repo-side-trial-dispatch-package.md"
LOCAL_PROVIDER_URL_DEFAULT = "http://127.0.0.1:8020"


@dataclass(frozen=True)
class DispatchSignalSpec:
    id: str
    path: Path
    required: bool = True
...
@dataclass(frozen=True)
class Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport:
    id: str
    generated_at: str
    status: str
    dispatch_state: str
...
SIGNAL_SPECS: list[DispatchSignalSpec] = [
    DispatchSignalSpec(
...
def build_phase15_myprivateagent_repo_side_trial_dispatch_package_report(
    *,
    base_dir: Path = Path("."),
) -> Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport:
    ...
    if any_missing_required or any_blocked_required:
        status = "blocked"
        dispatch_state = "blocked_for_repo_side_trial_dispatch"
        decision = "resolve_provider_contract_or_environment_blockers"
    elif any_review_required:
        status = "review"
        dispatch_state = "review_for_repo_side_trial_dispatch"
    ...
    return Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport(
        id=PHASE15_MYPRIVATEAGENT_REPO_SIDE_TRIAL_DISPATCH_PACKAGE_ID,
...
def phase15_myprivateagent_repo_side_trial_dispatch_package_report_to_dict(
    report: Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport,
) -> dict[str, Any]:
...
def render_phase15_myprivateagent_repo_side_trial_dispatch_package_markdown(
    report: Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport,
) -> str:
    lines = [
...
def export_phase15_myprivateagent_repo_side_trial_dispatch_package_report(
    *,
    output_dir: Path = Path("docs/integration/myprivateagent-repo-side-trial-dispatch"),
    base_dir: Path = Path("."),
) -> Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport:
    ...
    exported = Phase15MyPrivateAgentRepoSideTrialDispatchPackageReport(
        id=report.id,
    ...
    json_path.write_text(
        json.dumps(
            phase15_myprivateagent_repo_side_trial_dispatch_package_report_to_dict(
                exported
            ),
            ensure_ascii=False,
            indent=2,
...
def _blocker_category(*, signals: list[DispatchSignal], base_dir: Path) -> str:
    signal_map = {signal.id: signal for signal in signals}
    ...
    if any(signal_map[signal_id].status != "ready" for signal_id in phase10_11_13_14_ids):
...
def _caller_checklist(*, blocker_category: str, status: str) -> list[str]:
    ...
    if blocker_category == "handoff_visibility":
        return [
            ...
            "rerun_phase15_repo_side_trial_dispatch_package",
        ]
    if blocker_category == "external_environment":
        return [
    ...
    return [
        "regenerate_phase10_phase11_phase13_evidence",
        "rerun_phase15_repo_side_trial_dispatch_package",
    ]
