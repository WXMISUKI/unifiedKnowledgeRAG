...
def test_split_refund_real_source_can_pass_minimal_baseline(tmp_path):
    ...
    case_file = output_root / "split_refund_policy_docs" / "baseline-pack.fixture.json"
    case_file.write_text(
        Path(
            "docs/local-run/business-rag-golden-cases/onboarding/"
            "split_refund_policy_docs/baseline-pack.fixture.json"
...
def test_split_refund_source_is_provider_visible():
    ...
    assert "split_refund_policy_docs" in ids

    manifest = get_source_document_manifest("split_refund_policy_docs")
    assert manifest.ok is True
    assert manifest.result is not None
    assert manifest.result.documents[0].document_id == "split_refund_policy_2026"
